...
import { enable2faData, recoveryCodesDownloadData, confirm2faData, disable2faData } from './2fa';
import { tiptapEditorData } from './tiptapEditor';
import { confirmPasswordData } from './confirmPassword';

document.addEventListener('alpine:init', () => {
    window.Alpine.store('confirmPassword', {
        open: false,
        runAfterConfirm: null,
    });

    window.Alpine.data('tiptapEditor', tiptapEditorData);
    window.Alpine.data('modal', modalData);
    window.Alpine.data('enable2fa', enable2faData);
    window.Alpine.data('recoveryCodesDownload', recoveryCodesDownloadData);
    window.Alpine.data('confirm2fa', confirm2faData);
    window.Alpine.data('confirmPassword', confirmPasswordData);
    window.Alpine.data('disable2fa', disable2faData);
});
...
/**
 * Copy text to clipboard. Uses Clipboard API if available, else execCommand fallback.
 * Usage: window.copyToClipboard(text, successMessage?)
 */
window.copyToClipboard = function (text, successMessage) {
    if (!text) return;
    const done = () => {
        if (successMessage) {
            window.dispatchEvent(new CustomEvent('toast', { detail: { message: successMessage } }));
        }
    };
